...
Assessment emphasizes both computational practice and individual written understanding. Lab exercises use Python, in-class quizzes provide short checks of ongoing understanding, and the midterm and final examinations are \ul{written assessments completed without a Python environment}. % unless an approved accommodation requires an alternative format.
...
Quizzes will generally be completed during class without electronic devices, internet access, generative-AI tools, or a Python environment unless otherwise specified by the instructor. The lowest quiz score will be dropped when calculating the in-class quiz grade.
...
Generative-AI tools are not permitted on in-class quizzes, the written midterm, or the written final exam unless explicitly authorized by the instructor.
